...
class Response_helper
{
	...
	public static function getData(){
		...
		$list = $CI->db->query("SELECT pg.chat_id, peng.nama_pasien, p.status_notif, peng.no_rm, p.id from pengembalian p 
		JOIN pengambilan peng ON p.id_pengambilan=peng.id JOIN pengguna pg ON peng.created_by=pg.id 
		WHERE p.tanggal_harus_kembali <= '$now'")->result_array();
		foreach ($list as $d) {
			// echo $d['no_rm']."<br>";
			// echo $d['nama']."<br>";
